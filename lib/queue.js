...
module.exports = (thorin, opt) => {
  ...
  /**
   * Returns the queue size (number of messages in it)
   * */
  queueObj.size = function GetQueueSize(opt) {
    let arg = Array.prototype.slice.call(arguments),
      fn = arg.pop();
    if (typeof fn === 'function') {
      doGetSize(opt, fn);
  ...
  function doGetSize(options, done) {
    ...
    sqsObj.getQueueAttributes(params, (e, data) => {
      if (e) return done(parseError(e));
      let count = null;
      try {
        count = parseInt(data.Attributes.ApproximateNumberOfMessages, 10);
        if (isNaN(count)) throw 1;
      } catch (e) {
      }
      done(null, count);
    });
